...
%\VignetteDepends{surveillance, lattice, spdep, gsl, colorspace, animation, gridExtra, scales}
...
knitr::opts_chunk$set(echo = TRUE, tidy = FALSE, results = 'markup',
                      ...
                      cache = FALSE, error = FALSE, message = FALSE)
...
\author{Sebastian Meyer\thanks{Author of correspondence: \email{Sebastian.Meyer@ifspm.uzh.ch}}\\University of Zurich \And
...
%%     Section~\ref{sec:hhh4:data} introduces the data and the associated \code{S4}-class
...
\begin{equation} \label{eqn:hhh4}
  ...
  \phi_{it} \sum_{j \ne i} w_{ji} \, Y_{j,t-1}
\end{equation}
and overdispersion parameters $\psi_i > 0$ such that the conditional
...
\emph{SurvStat@RKI}\footnote{\url{https://survstat.rki.de}} 
online service.
...
and already used as an example in the original \code{hhh4} model proposal
\citep[Section~3.2]{held-etal-2005}.
These data are contained in \pkg{surveillance} as
\code{data("measlesWeserEms")} -- an object of the \code{S4}-class \class{sts}
...
\code{data("measles.weser")} originally analyzed by \citet{held-etal-2005}.
...
@ 
The overall time-series plot in Figure~\ref{fig:measlesWeserEms1} reveals strong
seasonality in the data with slightly different patterns in the two years.
The spatial plot in Figure~\ref{fig:measlesWeserEms2} is a tweaked \code{spplot}
(package \CRANpkg{sp}) with colors from \CRANpkg{colorspace} \citep{R:colorspace} using
$\sqrt{}$-equidistant cut points handled by package \CRANpkg{scales}
\citep{R:scales}.

The default plot \code{type} is \code{observed ~ time | unit}, which shows
the district-specific time series (Figure~\ref{fig:measlesWeserEms15}):
...
Figure~\ref{fig:measlesFitted_basic} shows this for the six districts
...
print(xtable(
    surveillance:::functionTable("hhh4", functions=list(Other="oneStepAhead")),
...
$i$ in the year 2004.\footnote{%
  ...
  (\url{http://www.nlga.niedersachsen.de/portal/live.php?navigation_id=36791&article_id=135436&_psmand=20}).}
...
Figure~\ref{fig:measlesFit_neweights2} shows both the power-law model
...
Figure~\ref{fig:measlesFit_neweights1} for the power-law model:
...
(see Figures~\ref{fig:measlesWeserEms2} and~\ref{fig:measlesWeserEms15}).
...
$\sigma_\nu^2$.
We did not assume correlation between the three random intercepts, but this is
...
\code{ri(type = "car")}.
The estimated random effects can be extracted by the
\code{ranef}-method:
<<>>=
head(ranef(measlesFit_ri, tomatrix = TRUE), n = 3)
@ 
They can also be visualized in a map by the plot \code{type = "ri"}
...
For the autoregressive component in Figure~\ref{fig:measlesFit_ri_map1}, we see
a pronounced heterogeneity between the three western districts in blue and the
...
autoregressive factor $\lambda_i = \exp(\alpha^{(\lambda)} + \alpha_i^{(\lambda)}) =
\Sexpr{exp(fixef(measlesFit_ri)[1]+ranef(measlesFit_ri, tomatrix=TRUE)["03403",1])}$,
but it seems to import more cases from other districts than explained by its
population (Figure~\ref{fig:measlesFit_ri_map2}).
In Figure~\ref{fig:measlesFit_ri_map3}, the two districts without any reported
measles cases (03401 and 03405) appear in dark pink, which means that they
...
Note that the extra flexiblility of the random effects model comes at a price. 
First, the estimation runtime increases considerably from
\Sexpr{round(measlesFit_powerlaw[["runtime"]]["elapsed"], 1)} seconds for the
previous power-law model \code{measlesFit_powerlaw} to 
\Sexpr{round(measlesFit_ri[["runtime"]]["elapsed"], 1)} seconds with additional
random effects. Furthermore, we no longer obtain AIC values in the model
summary, since random effects invalidate simple AIC-based model comparisons.
Of course we can plot the fitted values %(Figure~\ref{fig:measlesFitted_ri})
and visually compare their quality
...
autoregression. Note that the decomposition of the estimated mean by district
...
However, for quantitative comparisons of model performance we have to resort to
...
competing models by proper scoring rules for count data \citep{czado-etal-2009}.
...
which is usually averaged over a suitable set of forecasts to obtain the mean
squared error. More elaborate scoring rules such as the logarithmic score
(``logs'') or the ranked probability score (``rps'') take into account the whole
predictive distribution to assess calibration and sharpness simultaneously.
The so-called Dawid-Sebastiani score (``dss'') is another option.
...
Thus, the most parsimonious initial model \code{measlesFit_basic}
gives the best one-week-ahead predictions in terms of overall mean scores.
...
<<measlesPreds2_plot, echo=FALSE, eval=FALSE, purl=FALSE>>=
## plot observed counts and one-week-ahead predictions for selected districts
par(mfrow=c(2,1))
local(for (unit in c("03401", "03454")) {
    y <- measlesPreds2[[1]]$observed
    weeks <- as.integer(rownames(y))
    plot(weeks, y[,unit], type="h", ylab="No. infected", xlab="Week",
         ylim=c(0,1.05*max(y[,unit])))
    matlines(weeks, sapply(measlesPreds2, function (preds) preds$pred[,unit]),
             type="l", col=2:6, lty=c(2,1,2,1,1))
    legend("top", ncol=5, legend=sub("^measlesFit_", "", names(measlesPreds2)),
           col=2:6, lty=c(2,1,2,1,1), bty="n", cex=0.9)
})
@ 
...
epidemic was in the second quarter, while it already occured in the first
quarter in 2002 (cp.\ Figure~\ref{fig:measlesWeserEms1}).
...
compared to the long-term forecast:
...
knitr::write_bib(
    c("MASS", "Matrix", "spdep", "colorspace", "scales", "gridExtra", "lattice", "sp", "animation"),
...
\vfill
\section*{\proglang{R} version and packages used for this vignette}

<<sessionInfo, echo = FALSE, results = "asis", purl=FALSE>>=
