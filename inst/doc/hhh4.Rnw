...
disease counts.  The function is currently incorporated in the
development version of \surveillance\ available from
R-Forge\footnote{\url{http://surveillance.r-forge.r-project.org/}}.
...
S3 class data structure \code{disProg}.  They can be easily converted
...
<<nhoodByBw, fig=TRUE, echo=F, width=4,height=4>>=
# read in observed number of cases
...
%\vspace*{-2em}
\begin{center}
<<flu-ByBw, fig=T,width=6,height=6, include=T,echo=T>>=
par(mar=c(0,0,0,0))
...
\begin{center}
<<fit_men, fig=T, width=8,height=5>>=
