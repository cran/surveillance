...
among a group of army-soldiers in boot-camp~\citep{bulletin3901}.
%% In \surveillance\ the \texttt{readData} function is used to bring the
%% time series on \texttt{disProg} form. The SurvStat@RKI database
%% uses a 53 weeks a
...
by~\citet{held-etal-2005} is currently available. In the near future the
...
<<mapWeserEms, fig=TRUE, width=5, height=5, echo=FALSE>>=
...
<<fig=T,width=10,height=6>>=
plot(measles.weser,as.one=FALSE,xaxis.years=FALSE)
@
\vspace{1em}

The data are analysed using the model proposed by \citet{held-etal-2005}.
...
<<measles.hhh, echo=T, eval=F>>=
measles.hhh <- algo.hhh(measles.weser, control = cntrl)
@

Depending on the initial values for the parameters, the optimization algorithm
...
<<measles.hhh.grid, echo=T,eval=F>>=
