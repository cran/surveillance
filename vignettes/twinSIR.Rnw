...
components over time, we produce Figure~\ref{fig:hagellochFit_plot1} by:
...
The resulting Figure~\ref{fig:hagellochFit_plot2} reveals some deficiencies of
...
Rbib$quadprog$author <- "Berwin A. Turlach"
Rbib$quadprog$note <- paste0(Rbib$quadprog$note,
    ", ported to \\proglang{R} by Andreas Weingessel")
## write to bibfile
