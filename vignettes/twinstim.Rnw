...
%\VignetteDepends{surveillance, lattice, polyclip, memoise, maptools, spdep, colorspace, scales}
...
(secondary cases) around its origin $(\bm{s_j}, t_j)$
according to an inhomogeneous Poisson process with rate
$\eta_j \, f(\norm{\bm{s}-\bm{s_j}}) \, g(t-t_j)$.
...
and exponential decay of infectivity ($g(t) = e^{-\alpha t}$, $\tau_j
\equiv \infty$).
...
numerical integration. For this purpose, the \proglang{R}~package
\CRANpkg{polyCub} \citep{R:polyCub} offers cubature methods for polygonal
domains as described in \citet[Supplement~B, Section~2]{meyer.held2013}.
...
we established a cubature method which takes
advantage of the assumed isotropy of spatial interaction
such that numerical integration remains in only one dimension
\citep[Supplement~B, Section~2.4]{meyer.held2013}.
We also \CRANpkg{memoise} \citep{R:memoise} the cubature function during
...
See the \code{spTransform}-methods in package \CRANpkg{rgdal} \citep{R:rgdal}
...
the polygon complexity, e.g., by applying one of the simplification methods
available at \href{www.MapShaper.org}{MapShaper.org}.
...
print(xtable(
    surveillance:::functionTable("epidataCS", list(Convert=c("epidataCS2sts"))),
...
@ 
The time-series plot (Figure~\ref{fig:imdepi_plot1}) shows the monthly aggregated number of cases by finetype in
a stacked histogram as well as each type's cumulative number over time.
The spatial plot (Figure~\ref{fig:imdepi_plot2}) shows the observation window \code{W} with the locations of all
...
discussed by \citet{meyer.held2013}, but skipped here for the sake of brevity.
...
Last but not least, \class{epidataCS} can be converted to the other classes
\class{epidata} (described in \code{vignette("twinSIR")})
and \class{sts} (described in, e.g., \code{vignette("hhh4_spacetime")})
by aggregation.
...
The data could then, e.g., be analyzed by the
areal time-series model presented in \code{vignette("hhh4\_spacetime")}.
We can also use visualization tools of the \class{sts} class, e.g., to produce
Figure~\ref{fig:imdsts_plot}:
...
Coefficient correlations can be retrieved by the argument
\code{correlation = TRUE} in the \code{summary} call just like for
\code{summary.glm}, but may also be extracted via the standard
...
for some parameters (others start at 0) and set
\code{control.siaf} with a rather low number of nodes for the cubature
of $f(\norm{\bm{s}})$ in the log-likelihood (via the midpoint rule) and
$\frac{\partial f(\norm{\bm{s}})}{\partial \log\sigma}$
in the score function (via product Gauss cubature).
%over the individual influence regions~\eqref{eqn:twinstim:IR}
On Unix-alikes, these numerical integrations can be performed in parallel 
using the ``multicore'' functions
...
\code{xtable} method \citep{R:xtable},
which provides rate ratios for the endemic and epidemic effects. 
...
save(imdfit_Gaussian, imdfit_powerlaw, imdfit_step4,
...
\code{plot(imdfit_Gaussian, which = "siaf")}.
...
@ 
\citet{meyer.held2013} found that a power-law decay of spatial interaction is
more appropriate than a Gaussian kernel to describe the spread of human infectious
diseases.
The power-law kernel concentrates on short-range interaction, but also exhibits a
...
To use the power-law kernel $f(x) = (x+\sigma)^{-d}$, we switch to the
prepared \class{epidataCS} object with \code{eps.s = Inf} and update the
previous Gaussian model as follows:
<<imdfit_powerlaw_echo, echo=TRUE, eval=FALSE, purl=FALSE>>=
<<imdfit_powerlaw>>
@ 
The estimated power-law parameters $(\hat\sigma, \hat d)$ are:
<<>>=
...
suggests superiority of the power-law vs.\ the Gaussian model and the
endemic-only model. The more flexible step function yields the best AIC value
...
The following code runs a single simulation over the last year
...
Rbib0 <- knitr::write_bib(
    c("polyCub", "memoise", "sp", "rgdal", "polyclip",
      ## spatstat, # non-standard author entries
      "maptools", "animation", "xtable"),
    file = NULL, tweak = FALSE, prefix = "R:")
## package spatstat yields a bad automatic bib entry
Rbib1 <- sapply(c("spatstat"), function (pkg) {
    bib <- citation(pkg, auto = TRUE)
    bib$key <- paste("R", pkg, sep=":")
    bib
}, simplify=FALSE, USE.NAMES=TRUE)
Rbib1$spatstat$author <- "Adrian Baddeley and Rolf Turner and Ege Rubak"
## write to bibfile
.Rbibfile <- file("twinstim-R.bib", "w", encoding = "latin1")
cat(unlist(c(Rbib0, lapply(Rbib1, toBibtex)), use.names = FALSE),
    file = .Rbibfile, sep = "\n")
close(.Rbibfile)
@
